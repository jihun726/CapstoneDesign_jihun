...
while True:
    ...
    for i, contour in enumerate(contours):
        if cv2.contourArea(contour) > 500:  # 작은 노이즈 무시
            ...
            # 사람의 중심 좌표와 너비, 높이 기록
            person_data_records[f"Person {people_count}"] = {"center": (cx, cy), "width": w, "height": h}
    ...
    # 실시간으로 사람의 중심 좌표, 너비, 높이 기록을 콘솔에 출력
    for person, data in person_data_records.items():
        print(f"{person}: Center coordinates {data['center']}, Width: {data['width']}, Height: {data['height']}")
