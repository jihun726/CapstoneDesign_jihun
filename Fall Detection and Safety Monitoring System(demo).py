...
while True:
    ...
    cv2.drawContours(red_outline, contours, -1, (0, 0, 255), 2)  # 빨간색(BGR: (0, 0, 255)) 테두리

    # 화재가 감지된 영역만 강조하여 최종 결과 이미지 생성
    result_image = cv2.addWeighted(gray_rgb_colored, 1.0, red_outline, 0.7, 0)
    ...
    if fall_detected:
        if fall_detected_time is None:
            ...
            cv2.putText(result_image, "ALARM: Fall Detected!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            trigger_buzzer()
